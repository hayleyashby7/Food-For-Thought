		...
		try {
			...
			if (!response.ok) {
				...
				if (data && data.details) {
					const errorMessage = data.details.map((detail: { message: string }) => detail.message).join(' ');
	...
	return (
		<div className='bg-yellow-100'>
			<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
				{meals.map((meal) => (
					<div key={meal.id} className='rounded overflow:auto shadow-md bg-yellow-200 p-4 border-4 border-orange-400 h-96'>
						<img src={`https://spoonacular.com/recipeImages/${meal.id}-240x150.${meal.imageType}`} alt={meal.title} className='w-full object-cover h-48' />
						<div className='p-4'>
