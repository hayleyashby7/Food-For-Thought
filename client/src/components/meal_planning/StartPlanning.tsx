...
const StartPlanning: React.FC = () => {
	...
	const [mealResponse, setMealResponse] = useState<MealPlanResponse>({ status: 0, meals: [], nutrients: { calories: 0, carbohydrates: 0, fat: 0, protein: 0 } });

	const resetPlan = () => {
		setMealRequest({ calories: '', diet: '', remove: '' });
		setMealResponse({ status: 0, meals: [], nutrients: { calories: 0, carbohydrates: 0, fat: 0, protein: 0 } });
		setFormActive(false);
	};
